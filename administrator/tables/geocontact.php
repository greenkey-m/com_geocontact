 ...
 * Geocontact table
 */
class GeocontactTableGeocontact extends JTable {
    ...
    public function bind($array, $ignore = '') {
        ...
        if (($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state', 'com_geocontact') && $array['state'] == 1)) {
            $array['state'] = 0;
        }
        ...
        if (!JFactory::getUser()->authorise('core.admin', 'com_geocontact.geocontact.' . $array['id'])) {
            $actions = JFactory::getACL()->getActions('com_geocontact', 'geocontact');
            $default_actions = JFactory::getACL()->getAssetRules('com_geocontact.geocontact.' . $array['id'])->getData();
            $array_jaccess = array();
        ...
        //Added for tags
        //#################################################################################
        if (isset($array['tagfieldname']) && !empty($array['tagfieldname'])) {
